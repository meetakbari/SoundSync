...
public class hdfs_write_api extends BaseAPIViewSet {
    ...
    public void post(HttpReq request) throws Exception {
        ...
        try
        {  
            String song_name = request.FormData.getOrDefault("song_name", "Not Added!").toString().replaceAll("(\\r|\\n)", "");
            String Album_song = request.FormData.getOrDefault("Album", "Not Added!").toString().replaceAll("(\\r|\\n)", "");
            String Composer = request.FormData.getOrDefault("Composer", "Not Added!").toString().replaceAll("(\\r|\\n)", "");
            List<String> Featuring = Arrays.asList(request.FormData.getOrDefault("Featuring", "Not Added!").toString().replaceAll("(\\r|\\n)", "").split("\\s*,\\s*"));
            List<String> Genre = Arrays.asList(request.FormData.getOrDefault("Genre", "Not Added!").toString().replaceAll("(\\r|\\n)", "").split("\\s*,\\s*"));
            List<String> Lyricist = Arrays.asList(request.FormData.getOrDefault("Lyricist", "Not Added!").toString().replaceAll("(\\r|\\n)", "").split("\\s*,\\s*"));
            String Release_date = request.FormData.getOrDefault("Release_date", "Not Added!").toString().replaceAll("(\\r|\\n)", "");
            String Language = request.FormData.getOrDefault("Language", "Not Added!").toString().replaceAll("(\\r|\\n)", "");
            List<String> Singers = Arrays.asList(request.FormData.getOrDefault("Singer", "Not Added!").toString().replaceAll("(\\r|\\n)", "").split("\\s*,\\s*"));
            ...
            Featuring.get(Featuring.size() - 1).replaceAll("(\\r|\\n)", "");
            Genre.get(Genre.size() - 1).replaceAll("(\\r|\\n)", "");
            Lyricist.get(Lyricist.size() - 1).replaceAll("(\\r|\\n)", "");
            Singers.get(Lyricist.size() - 1).replaceAll("(\\r|\\n)", "");
            ...
            String hdfsDir = "hdfs://10.20.24.32:9000/song_store" + "/" + metaDataDoc.get("_id") + "." +a.get(0).FileName.substring(a.get(0).FileName.lastIndexOf(".")+1);
